...
const getHuman = async () => {
  ...
  // Dynamic import so it never gets bundled into native
...
export default function KioskWebScreen() {
    ...
    const init = async () => {
      try {
        ...
        const branchWorkers = fetchedWorkers
          .map((w) => {
            // Normalise faceDescriptor (Firestore may return object instead of array)
          ...
          .filter((w) =>
            (w.branchId ?? 'default') === (kioskConfig.branchId || 'default') &&
            w.status === 'ACTIVE'
          );
        ...
        setFeedback(`Ready · ${branchWorkers.filter((w) => (w as any).faceDescriptor?.length > 0).length} faces loaded`);
      } catch (e) {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(async () => {
      ...
      try {
        ...
        // Match against workers with face descriptors
  ...
  const handlePunch = useCallback(async (worker: Worker, method: 'Face Scan' | 'QR Badge') => {
    if (processingRef.current || !kioskConfig) return;
    processingRef.current = true;
    ...
    try {
      ...
      // Cooldown check
      const cooldownSecs = await checkCooldown(kioskConfig.tenantId, worker.id, today);
      ...
      await handleLeaveCancellation(kioskConfig.tenantId, worker.id, today, worker);

      // Determine IN/OUT
      const punchType = await determinePunchType(kioskConfig.tenantId, worker.id, today);

      const now = new Date();
      const punch: OfflinePunch = {
        id: `${kioskConfig.tenantId}_${worker.id}_${now.getTime()}`,
        tenantId: kioskConfig.tenantId,
        workerId: worker.id,
      ...
      // Try online, fall back to offline queue
  ...
  const handlePinSubmit = async () => {
    if (!kioskConfig) return;
    if (pinInput === kioskConfig.adminPin) {
      streamRef.current?.getTracks().forEach((t) => t.stop());
  ...
  if (configLoading) return (
    <div style={css.fullCenter}>
      <p style={css.loadingTxt}>Loading kiosk...</p>
    </div>
  );

  if (!kioskConfig) return (
    <div style={css.fullCenter}>
      <p style={css.loadingTxt}>Kiosk not configured.</p>
      <p style={css.loadingSub}>Pair this device from Settings → Terminals.</p>
      <button style={css.setupBtn} onClick={() => router.replace('/(admin)/settings')}>
        Go to Settings
      </button>
    </div>
  );
    ...
    <div style={css.root}>
      ...
      <div style={css.cameraPane}>
        ...
        <div style={css.topBar}>
          <div>
            <p style={css.orgName}>{kioskConfig.orgName}</p>
            <p style={css.branchName}>{kioskConfig.branchName} · {kioskConfig.terminalName}</p>
          </div>
        ...
        <div style={css.statusBar}>
          ...
          {phase === 'OFFLINE' && (
            <div style={css.offlinePill}>
              ☁ Offline — punch saved locally
            </div>
          )}
        </div>
        ...
        {successData && (phase === 'SUCCESS' || phase === 'OFFLINE') && (
          <div style={css.successOverlay}>
            <div style={css.successCard}>
              <div style={{ ...css.successAvatar, backgroundColor: successData.punchType === 'IN' ? '#22C55E' : '#EF4444' }}>
                <span style={css.successInitial}>{successData.name.charAt(0).toUpperCase()}</span>
              </div>
              <p style={css.successName}>{successData.name}</p>
              <div style={{ ...css.successBadge, backgroundColor: successData.punchType === 'IN' ? '#DCFCE7' : '#FEE2E2' }}>
                <span style={{ ...css.successBadgeTxt, color: successData.punchType === 'IN' ? '#15803D' : '#DC2626' }}>
        ...
        <button
          style={css.exitBtn}
          onClick={() => { setShowExitPin(true); processingRef.current = true; setPinInput(''); setPinError(''); }}
        >
          🔒 Exit
        </button>
      </div>
      ...
      <div style={css.activityPane}>
        <div style={css.activityHeader}>
          ...
          <p style={css.activitySub}>
            <span style={css.onlineDot} /> System Active
          </p>
        </div>
      ...
      {/* ── PIN exit modal ── */}
      {showExitPin && (
        <div style={css.pinOverlay}>
          <div style={css.pinCard}>
            <p style={css.pinTitle}>🛡 Exit Kiosk Mode</p>
            <p style={css.pinSub}>Enter Admin PIN to close terminal</p>
            <input
              type="password"
              maxLength={4}
              value={pinInput}
              ...
              placeholder="••••"
            />
            {!!pinError && <p style={css.pinError}>{pinError}</p>}
            <div style={css.pinBtns}>
              <button
                style={css.pinCancelBtn}
                onClick={() => { setShowExitPin(false); processingRef.current = false; setPinInput(''); setPinError(''); }}
              >
                Cancel
              </button>
              <button style={css.pinConfirmBtn} onClick={handlePinSubmit}>
                Unlock
              </button>
            </div>
          </div>
...
const css: Record<string, React.CSSProperties> = {
  ...
  loadingSub:   { color: '#9CA3AF', fontSize: 14, textAlign: 'center', maxWidth: 320 },
  ...
  onlineDot:    { display: 'inline-block', width: 8, height: 8, borderRadius: 4, backgroundColor: '#22C55E', animation: 'pulse 2s infinite' },
  ...
  // PIN
  pinOverlay:   { position: 'fixed', inset: 0, backgroundColor: 'rgba(0,0,0,0.9)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 50 },
  pinCard:      { backgroundColor: '#1F2937', borderRadius: 24, padding: 36, width: 360, textAlign: 'center', boxShadow: '0 20px 50px rgba(0,0,0,0.5)' },
  pinTitle:     { color: '#fff', fontSize: 20, fontWeight: 900, margin: '0 0 8px' },
  pinSub:       { color: '#9CA3AF', fontSize: 13, margin: '0 0 20px' },
  pinInput:     { width: '100%', textAlign: 'center', fontSize: 28, letterSpacing: 12, padding: '14px 0', backgroundColor: '#374151', border: 'none', borderRadius: 14, color: '#fff', outline: 'none', boxSizing: 'border-box' },
  pinError:     { color: '#EF4444', fontSize: 13, fontWeight: 600, margin: '10px 0 0' },
  pinBtns:      { display: 'flex', gap: 10, marginTop: 20 },
  pinCancelBtn: { flex: 1, padding: 14, backgroundColor: '#374151', color: '#D1D5DB', border: 'none', borderRadius: 14, fontWeight: 700, fontSize: 14, cursor: 'pointer' },
  pinConfirmBtn:{ flex: 1, padding: 14, backgroundColor: '#DC2626', color: '#fff', border: 'none', borderRadius: 14, fontWeight: 700, fontSize: 14, cursor: 'pointer' },
};
